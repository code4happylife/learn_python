...
def demo_string():
    ...
    print stra.replace('python', 'ruby')
    strb = '   \n\rhello world \r\n'
    print 1, strb.lstrip()
    print 2, strb.rstrip()
    strc = 'hello world'
    ...
    print 7, len(stra)
    print 8, '-'.join(['a', 'b', 'c'])
    print 9, '*'.join(['usa', 'prc', 'uk'])
    print 10, stra.split(' ')
    print 11, stra.find('hello')
    print 12, stra.find('ello')
...
def demo_buildinfunction():
    print 1, max(2, 8), min(4, 9)
    print 2, len('xxx'), len([1, 2, 3])
    print 3, abs(-5)
    print 4, range(1, 20, 3)
    print 5, dir(list)
    x = 2
    print 6, eval('x+7')
...
def demo_dict():
    ...
    print 3, dicta.has_key(2), dicta.has_key('3')
    for key, value in dicta.items():
        print 'key-value:', key, value
    dictb = {'+': add, '-': sub}
    print 4, dictb['+'](1, 2)
    print 5, dictb['-'](3, 9)
    print 6, dictb.get('-')(15, 3)
    dictb['*'] = 'x'
    print 7, dictb
...
def create_user(type):
    if type == 'USER':
        ...
        return Admin('a1', 1, 'group A')
    else:
        # raise ValueError('error')
        return Guest('GUEST', 7)
...
def demo_exception():
    try:
        ...
        # print 2/0
        #if type=='c'
        raise Exception('Raise error', 'python parameter type')
...
def demo_random():
    ...
    print 3, random.choice([1, 2, 3, 4, 5])
    print 4, random.choice(range(0, 100, 10))
    print 5, random.sample(range(0, 100), 20)
    a = [76, 0, 72, 22, 94, 90, 3, 54]
    random.shuffle(a)
    print 6, a
